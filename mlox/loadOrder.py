...
def get_load_order():
    if "openmw.cfg" in os.listdir(os.getcwd()):
        return OpenMWLoadOrder()
    else:
        return MorrowindLoadOrder()


class LoadOrder(metaclass=ABCMeta):
    def __init__(self):
        ...
        for i in range(0, len(self.new_order)):
            ...
            if highlight == "*":
                if i < len(self.new_order) - 1:
                    next_plugin = self.new_order[i + 1].lower()
                if orig_index[curr] > orig_index[next_plugin]:
                    highlight = "_"
        return formatted

    def explain(self, plugin_name, base_only):
...
class MorrowindLoadOrder(LoadOrder):
    """Class for reading OG Morrowind plugin mod times (load order), and updating them based on rules"""
    ...
    def read_from_file(self, fromfile):
        ...
        self.is_sorted = False
        self.game_type = None
        # self.datadir tells the parser to not worry about things like [SIZE] checks,
        # or trying to read the plugin descriptions
        self.datadir = None
        self.plugin_file = fromfile

        self.order = configHandler.configHandler(fromfile).read()
        if self.order == []:
            order_logger.warning("No plugins detected.\nmlox understands lists of plugins in the format used by "
                                 "Morrowind.ini or Wrye Mash.\nIs that what you used for input?")

        # Convert the files to lowercase, while storing them in a dict
        self.order = list(map(self.caseless.cname, self.order))

        order_logger.info("Found {0} plugins in: \"{1}\"".format(len(self.order), self.plugin_file))
        order_logger.info("(Note: When the load order input is from an external source, the [SIZE] predicate cannot "
                          "check the plugin filesizes, so it defaults to True).")
    ...
    def update(self, progress=None):
        ...
        if len(self.new_order) != len(self.order):
            order_logger.error("sanity check: len(self.new_order %d) != len(self.order %d)",
                               len(self.new_order),
                               len(self.order))
            self.new_order = []
            return False
...
class OpenMWLoadOrder(LoadOrder):
    """Class for reading OpenMW configuration, and updating it based on rules"""
    def get_active_plugins(self):
        """
        Get the active list of plugins from the game configuration.

        "Active Plugins" are plugins that are listed in the `content=` of openmw.cfg
        Updates self.order
        """
        self.is_sorted = False
        ...
        # Get all the plugins
        self.order = configHandler.configHandler(self.plugin_file, self.game_type).read()
